...
export function registerRoutes(app: FastifyInstance) {
  ...
  app.post('/', { preHandler: [validateBody(createCustomerSchema)] }, async (request, reply) => {
    const result = await service.create(request.ctx.tenant!.organizationId, request.validatedBody as z.infer<typeof createCustomerSchema>);
    return reply.status(201).send({ data: result });
  });

  app.patch('/:id', { preHandler: [validateParams(idParamsSchema), validateBody(updateCustomerSchema)] }, async (request, reply) => {
    const { id } = request.validatedParams as z.infer<typeof idParamsSchema>;
    const result = await service.update(id, request.ctx.tenant!.organizationId, request.validatedBody as z.infer<typeof updateCustomerSchema>);
